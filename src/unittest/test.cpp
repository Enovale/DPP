...
Markdown lol \\|\\|spoiler\\|\\| \\~\\~strikethrough\\~\\~ \\`small \\*code\\* block\\`\n");
		...
		try {
			...
			if (!offline) {
				...
				try {
					c3 = new dpp::https_client(&bot, "dl.dpp.dev", 443, "/***REMOVED***test.php", "GET", "", {}, true, 5, "1.1", [](dpp::https_client *c2) {
						size_t count = c2->get_header_count("set-***REMOVED***");
						size_t count_list = c2->get_header_list("set-***REMOVED***").size();
						// This test script sets a bunch of ***REMOVED***s when we request it.
